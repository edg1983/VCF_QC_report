...
```
bcftools query -i 'FILTER == "PASS"' -f '[%CHROM\t%POS\t%SVTYPE\t%SVLEN\t%AF\t%SAMPLE\t%GT\t%SQ\t%DP\t%CN\n]' Thakker.classify.filters.vcf.gz > SV_per_sample_info.tsv
```

- Per variant data generated with
```
bcftools query -i 'FILTER == "PASS"' -H -f '%CHROM\t%POS\t%ID\t%FILTER\t%SVTYPE\t%SVLEN\t%CIPOS95\t%AF\n' Thakker.classify.filters.vcf.gz > SV_per_variant_info.tsv
```

```{r load_data, include=F}
vars <- read.csv(params$var_stats, header=T, sep="\t")
sample_data <- read.csv(params$sample_stats, header=F, sep="\t")
