...
p1 <- ggplot(canonical_chrs, aes(x=chrom, y=vars_per_Mb)) + 
  geom_bar(stat="identity") + x_axis_text_45 +
  scale_y_continuous(breaks = seq(0,max(canonical_chrs$vars_per_Mb),2000)) + theme_bw()
...
p2 <- ggplot(mt_chrom, aes(x=chrom, y=vars_per_kb)) + 
  geom_bar(stat="identity") + x_axis_text_45 +
  scale_y_continuous(breaks = seq(0,max(mt_chrom$vars_per_kb),2000)) + theme_bw()
...
ggplot(AF_df, aes(x=AF_bin, y=n_var/1000000)) + 
  ...
  x_axis_text_45 + labs(y="M vars", x="cohort AF") +
  theme_bw()
```
